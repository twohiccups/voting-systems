...
export default function WinnerCard({
    ...
    const votes = candidate.total ?? 0;

    const pct = (n: number, d: number) => (d === 0 ? 0 : Math.round((n / d) * 100));
    const percent =
        typeof percentOverride === "number" ? Math.round(percentOverride) : pct(votes, totalVotes);
            ...
            <div className="relative z-10 flex flex-col gap-3">
                ...
                <div>
                    ...
                    <div className="h-2 w-full overflow-hidden rounded-full bg-green-200 dark:bg-green-900/40">
                        <div
                            className="h-full rounded-full bg-green-600 transition-[width] duration-700"
                            style={{ width: `${percent}% ` }}
                        />
                    </div>
                </div>
                ...
                <p className="text-sm">
                    {candidate.label} wins with <strong>{votes.toLocaleString()}</strong> of{" "}
